...
import com.iyke.ozix.domain.model.SignalType
...
class ProcessSignalsUseCaseImpl @Inject constructor(
    ...
    private suspend fun refreshAllForNewMessages(chatIds: List<String>): MutableList<Message> {
        val thisUserId = thisUserRepo.get()?.userId ?: return mutableListOf()
        val newMessages = mutableListOf<Message>()
        for (chatId in chatIds) {
            if (chatId.length > 40) { // checking if it is a pair chat
                val chatMateId = getOtherParticipantId(chatId, thisUserId)
            ...
            val chatNewMessages = messageRepo.syncMessages(chatId)
            newMessages.addAll(chatNewMessages)
            chatRepo.syncChat(chatId, chatNewMessages.isNotEmpty())
        }
        return newMessages
    }
